...
float sum_par_scan_cu(const float *summands, const int n, const int num_threads)
{
    ...
    HANDLE_ERROR(cudaFuncSetAttribute(
        sum_par_scan, cudaFuncAttributeMaxDynamicSharedMemorySize, shared_memory_bytes
    ));
    ...
    // call kernel
    sum_par_scan<<<blocks_per_grid, threads_per_block, shared_memory_bytes>>>(
        gpu_summands, n, gpu_result
    );
    CudaCheckError();
...
float sum_par_cu(const float *summands, const int n, const int num_threads)
{
    ...
    sum_par<<<blocks_per_grid, threads_per_block, shared_memory_bytes>>>(
        gpu_summands, n, gpu_result
    );
    CudaCheckError();
